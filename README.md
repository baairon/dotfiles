...
files directly through its `dotfiles-setup` skill (which ethagent syncs onto each
machine), rather than keeping its own copies that would drift, so editing here is all
